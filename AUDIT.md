...
- **Half-close is never propagated, so connections leak.** `main.rs:399`: `tokio::try_join!(client_to_server, server_to_client)` waits for **both** directions.

  *Correction, from testing in A4:* the direction stated in the original audit was wrong. When the **client** disappears the upstream does close, because the completed forwarding future drops the upstream write half and PostgreSQL sees EOF; a test written to prove that leak passed before any change was made. The real leak is the reverse: when **PostgreSQL** closes, the client is never told and its read side sits there until the operating system's TCP timeout. Fixed in A4.
...
| 2 — `CancelRequest` unhandled and corrupted | **fixed** | `00eda16` | Reproduced at both levels: fake backend saw `declared_len` 40 instead of 16; real PostgreSQL reported `the query was never cancelled`. Now relayed verbatim, and deliberately exempt from Guardian connection rules. CI green. |
